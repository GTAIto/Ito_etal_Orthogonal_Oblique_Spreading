...
plot(TFfrict_a,0.75*dsnorm2_a,'k-','Linewidth',anal_lw,'Color',anal_color); hold on;
plot(TFfrict_a,dsnorm2_a,'k-','Linewidth',anal_lw,'Color',anal_color);
plot(TFfrict_a,1.25*dsnorm2_a,'k-','Linewidth',anal_lw,'Color',anal_color);
% plot(TFfrict_a,1.25*dsnorm2_a,'k-','Color',anal_color,'Linewidth',anal_lw); 
% plot(TFfrict_a,0.75*dsnorm2_a,'k-','Color',anal_color,'Linewidth',anal_lw); hold on;
plot(TFfrict(ii),dsnorm2(ii),'ko','MarkerFaceColor',blu,'Markersize',12); hold on;
%ylabel('\Delta\sigma/\sigma_v_a_r, MPa'); 
axis([0 0.65 0.0 0.5]);
set(gca,'Fontsize',fsz);
set(gca,'TickLength',[tl tl],'Linewidth',lw);
%xlabel('Transform friction coefficient, \mu_T_F'); 

subplot('position',[x1+(xw+xbuff) ytop xw yh])
plot(xmod,-polyval(pf,xmod),'k-', 'Linewidth', 1.5); hold on;
plot(dsnorm2(ii),-dwdt(ii),'ko','MarkerFaceColor',blu,'Markersize',12); hold on;
set(gca,'Fontsize',fsz);
%xlabel('\Delta\sigma/\sigma_v_a_r');
xticks([0.1:0.1:0.4])
axis([0.05 0.45 0 30]);
set(gca,'TickLength',[tl tl],'Linewidth',lw);
%grid on;
...
if (0)
    ...
    exportgraphics(gcf,['Fig8_vary_wTF.png'],'Resolution',600)
else
